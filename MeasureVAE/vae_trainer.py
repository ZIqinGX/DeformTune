...
class VAETrainer(Trainer):
    ...
    def compute_reg_loss(self, z, score):
        ...
        x = z[:, 0]
        reg_loss = self.reg_loss_sign(x, attr_tensor) * 5.0
        return reg_loss
    ...
    def train_model(self, batch_size=128, num_epochs=10, plot=False, log=False):
        print("🔥 train_model() 开始执行")
        ...
        print(f"📊 DataLoader 中的样本数量: {len(train_loader.dataset)}，Batch 数量: {len(train_loader)}")
        ...
        for epoch in range(num_epochs):
            ...
            for batch in train_loader:
                ...
                loss, acc = self.loss_and_acc_for_batch(batch, epoch, train=True)
